...
const Bloom = ({ x, y, c, type, delay }) => {
  ...
  if (type === 'daisy') {
    const petals = Array.from({ length: 7 }, (_, i) => {
      ...
      return <ellipse key={i} cx={px} cy={py} rx={6} ry={9}
               fill={color} transform={`rotate(${a} ${px} ${py})`} />
    })
    return (
  ...
  if (type === 'spike') {
    const ovals = Array.from({ length: 8 }, (_, i) => (
      <ellipse key={i} cx={x + (i % 2 === 0 ? -3 : 3)} cy={y - i * 7}
        rx={4} ry={4.5} fill={color} opacity={0.85 - i * 0.04} />
    ))
...
const Flower = ({ flower, idx }) => {
  ...
  return (
    <g>
      <path className="stem" pathLength="1"
      ...
      {idx % 2 === 0 && (
        <>
          <ellipse className="leaf-tiny" cx={x - 7} cy={leafY1} rx={6} ry={3}
            fill="var(--leaf-1)" transform={`rotate(-30 ${x - 7} ${leafY1})`}
            style={{ '--cx': `${x - 7}px`, '--cy': `${leafY1}px`, '--delay': `${stemDelay + 0.4}s` }} />
          <ellipse className="leaf-tiny" cx={x + 7} cy={leafY2} rx={5} ry={2.5}
            fill="var(--leaf-2)" transform={`rotate(30 ${x + 7} ${leafY2})`}
            style={{ '--cx': `${x + 7}px`, '--cy': `${leafY2}px`, '--delay': `${stemDelay + 0.55}s` }} />
        </>
      )}
...
const GrassTuft = ({ x, dx, idx }) => {
  ...
  const h = 18 + (idx % 3) * 6
  return (
    <path className="grass-blade"
      d={`M ${x} ${baseY} Q ${x + dx} ${baseY - h * 0.6}, ${x + dx * 1.3} ${baseY - h}`}
      stroke="var(--leaf-2)" strokeWidth={1.6} strokeLinecap="round" fill="none"
      style={{ '--cx': `${x}px`, '--cy': `${baseY}px`, '--delay': `${0.5 + idx * 0.05}s`,
...
const Trunk = () => (
  <g>
    <path className="trunk-fill"
      ...
      fill="var(--bark)" style={{ transformOrigin: '700px 1000px' }} />
    <path className="trunk-fill"
      d="M 685 1000 C 686 940, 686 870, 690 800 C 694 730, 696 660, 698 580
         C 699 530, 700 490, 700 470"
      stroke="var(--bark-deep)" strokeWidth="2" fill="none" strokeLinecap="round"
...
const Branches = () => (
  <g>
    {BRANCHES.map((b, i) => (
      <path key={i} className="branch-stroke" d={b.d} pathLength="1" strokeWidth={b.w}
...
const CanopyBlob = ({ cx, cy, rx, ry, fill, delay, op }) => (
  <ellipse className="canopy-blob" cx={cx} cy={cy} rx={rx} ry={ry}
    fill={`var(--${fill})`}
...
const Canopy = () => (
  <g className="canopy-group">
    {CANOPY.map((c, i) => <CanopyBlob key={i} {...c} />)}
  </g>
)
...
const Grass = () => (
  <g>
    {GRASS.map((g, i) => <GrassTuft key={i} x={g.x} dx={g.dx} idx={i} />)}
  </g>
)
