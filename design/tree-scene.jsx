// Tree + wildflowers SVG scene
// viewBox: 0 0 1400 1000.  Divide line is at y = 1000 (bottom of svg).
// Tree base at (700, 1000), grows upward.  Wildflowers also rise from y = 1000.
...
// ---------- Tree branches ----------
// Each branch: a path string, base delay (s), draw duration (s), stroke width.
const BRANCHES = [
  // Low droop branches — these are the "fancy hanging" branches over wildflowers
  { d: 'M 690 730 C 600 740, 480 770, 360 850 C 300 890, 280 905, 260 915',
    delay: 1.05, dur: 1.6, w: 14, group: 'low' },
  { d: 'M 712 730 C 800 740, 920 770, 1040 850 C 1100 890, 1120 905, 1140 915',
    delay: 1.05, dur: 1.6, w: 14, group: 'low' },
  // sub-branches off the low boughs (twigs that hang down)
  { d: 'M 470 800 C 455 830, 445 855, 440 880',
    delay: 2.1, dur: 0.9, w: 5, group: 'low' },
  { d: 'M 380 838 C 380 870, 384 890, 388 905',
    delay: 2.2, dur: 0.9, w: 4, group: 'low' },
  { d: 'M 930 800 C 945 830, 955 855, 960 880',
    delay: 2.1, dur: 0.9, w: 5, group: 'low' },
  { d: 'M 1020 838 C 1020 870, 1016 890, 1012 905',
    delay: 2.2, dur: 0.9, w: 4, group: 'low' },
  // sub-branches that curl back upward off the low boughs
  { d: 'M 540 760 C 510 720, 470 690, 420 660',
    delay: 1.9, dur: 1.0, w: 7, group: 'low' },
  { d: 'M 860 760 C 890 720, 930 690, 980 660',
    delay: 1.9, dur: 1.0, w: 7, group: 'low' },

  // Mid branches — broader horizontal reach
  { d: 'M 686 600 C 600 580, 480 540, 360 470 C 320 450, 290 440, 260 432',
    delay: 1.35, dur: 1.55, w: 12, group: 'mid' },
  { d: 'M 716 600 C 802 580, 920 540, 1040 470 C 1080 450, 1110 440, 1140 432',
    delay: 1.35, dur: 1.55, w: 12, group: 'mid' },
  // mid sub-branches
  { d: 'M 480 540 C 460 510, 440 480, 420 460',
    delay: 2.4, dur: 0.9, w: 6, group: 'mid' },
  { d: 'M 920 540 C 940 510, 960 480, 980 460',
    delay: 2.4, dur: 0.9, w: 6, group: 'mid' },
  { d: 'M 600 580 C 580 540, 560 510, 540 488',
    delay: 2.5, dur: 0.8, w: 5, group: 'mid' },
  { d: 'M 800 580 C 820 540, 840 510, 860 488',
    delay: 2.5, dur: 0.8, w: 5, group: 'mid' },

  // Upper diagonal branches — reach upward & outward
  { d: 'M 680 490 C 620 440, 560 380, 500 320 C 470 290, 450 270, 430 252',
    delay: 1.65, dur: 1.45, w: 10, group: 'up' },
  { d: 'M 720 490 C 780 440, 840 380, 900 320 C 930 290, 950 270, 970 252',
    delay: 1.65, dur: 1.45, w: 10, group: 'up' },
  // crown branches — top
  { d: 'M 696 470 C 670 380, 640 290, 610 200',
    delay: 1.85, dur: 1.4, w: 9, group: 'up' },
  { d: 'M 704 470 C 730 380, 760 290, 790 200',
    delay: 1.85, dur: 1.4, w: 9, group: 'up' },
  // upper sub-branches
  { d: 'M 580 360 C 555 320, 530 280, 510 250',
    delay: 2.7, dur: 0.85, w: 5, group: 'up' },
  { d: 'M 820 360 C 845 320, 870 280, 890 250',
    delay: 2.7, dur: 0.85, w: 5, group: 'up' },
  { d: 'M 660 320 C 640 270, 625 230, 615 200',
    delay: 2.85, dur: 0.85, w: 4, group: 'up' },
  { d: 'M 740 320 C 760 270, 775 230, 785 200',
    delay: 2.85, dur: 0.85, w: 4, group: 'up' },
];

// ---------- Canopy blobs (organic foliage clouds) ----------
// Each blob is a path describing a soft organic shape, with a center for transform-origin.
const CANOPY = [
  // Background pass — light layer, large blobs
  ...
  // Drooping outer puffs that hang over wildflowers
  ...
  // Highlight pass — small light blobs on top for sparkle
...
// ---------- Wildflowers ----------
// Each flower: x position, height, stem curve, bloom type, color.
function buildFlowers(palette, density) {
  const palettes = FLOWER_PALETTES[palette] || FLOWER_PALETTES.spring;
  // Strategically distributed; lighter near trunk (x≈700), denser at edges
  const positions = [
  ...
  // Density filter
  const step = density === 'lush' ? 1 : density === 'sparse' ? 2 : 1;
  const filtered = density === 'sparse'
    ? positions.filter((_, i) => i % 2 === 0)
    : positions;

  return filtered;
}

// Bloom rendering by type
const Bloom = ({ x, y, c, type, delay }) => {
  ...
  if (type === 'cluster') {
    // Queen anne's lace — small dot cluster
    const dots = [];
    const positions = [
      [0, 0], [-7, -3], [7, -3], [-4, -9], [4, -9], [0, -12],
      [-10, 2], [10, 2], [-6, 6], [6, 6], [0, 7], [-12, -6], [12, -6],
    ];
    positions.forEach(([dx, dy], i) => {
      dots.push(
        <circle key={i} cx={x + dx} cy={y + dy} r={2.6} fill={color} />
      );
    });
    return <g className="bloom" style={style}>{dots}</g>;
  }

  if (type === 'spike') {
    // Lavender-style spike — small ovals up the stem (pointing into stem)
    const ovals = [];
    for (let i = 0; i < 8; i++) {
      const dy = -i * 7;
      const dx = (i % 2 === 0 ? -3 : 3);
      ovals.push(
        <ellipse key={i} cx={x + dx} cy={y + dy} rx={4} ry={4.5}
          fill={color} opacity={0.85 - i * 0.04} />
      );
    }
    return <g className="bloom" style={style}>{ovals}</g>;
  }
  ...
  if (type === 'bell') {
    ...
    return <g className="bloom" style={style}>{bells}</g>;
  }

  return null;
};

const Flower = ({ flower, idx, density }) => {
  ...
  // Stagger appearance across the row
  const stemDelay = 1.0 + idx * 0.08;
  const bloomDelay = stemDelay + 0.8;

  // Two leaves on the stem for some flowers
    ...
    <g>
      ...
      {(idx % 2 === 0) && (
        <>
          <ellipse
            className="leaf-tiny"
            cx={x - 7} cy={leafY1} rx={6} ry={3}
            fill="var(--leaf-1)"
            transform={`rotate(-30 ${x - 7} ${leafY1})`}
            style={{
              ['--cx']: `${x - 7}px`, ['--cy']: `${leafY1}px`,
              ['--delay']: `${stemDelay + 0.4}s`,
            }}
          />
          <ellipse
            className="leaf-tiny"
            cx={x + 7} cy={leafY2} rx={5} ry={2.5}
            fill="var(--leaf-2)"
            transform={`rotate(30 ${x + 7} ${leafY2})`}
            style={{
              ['--cx']: `${x + 7}px`, ['--cy']: `${leafY2}px`,
...
// ---------- Grass tufts at base ----------
const GRASS = [
  { x: 540, dx: -8 }, { x: 555, dx: 4 }, { x: 570, dx: -6 },
  { x: 720, dx: 6 },  { x: 735, dx: -3 }, { x: 750, dx: 5 }, { x: 765, dx: -8 },
  { x: 600, dx: 2 },  { x: 640, dx: -5 }, { x: 660, dx: 7 },  { x: 685, dx: -4 },
  { x: 800, dx: 3 },  { x: 820, dx: -7 },
];
...
// ---------- Trunk shape (filled, tapered) ----------
const Trunk = () => (
  <g>
    {/* Main trunk silhouette — base wider, top narrow, slight S-curve */}
    <path
      className="trunk-fill"
      d="
        M 670 1000
        C 668 950, 665 900, 670 850
        C 674 800, 678 760, 686 720
        C 690 680, 690 640, 692 600
        C 694 560, 695 520, 696 470
        L 704 470
        C 705 520, 706 560, 708 600
        C 710 640, 710 680, 714 720
        C 722 760, 726 800, 730 850
        C 735 900, 732 950, 730 1000
        Z"
      fill="var(--bark)"
    ...
    <path
      ...
      d="
        M 685 1000
        C 686 940, 686 870, 690 800
        C 694 730, 696 660, 698 580
    ...
    <path
      ...
      fill="var(--bark)"
      style={{ transformOrigin: '630px 1000px', animationDelay: '0.05s', animationDuration: '0.9s' }}
    />
    <path
      className="trunk-fill"
      d="M 800 1000 C 770 990, 740 985, 730 985 L 730 1000 Z"
      fill="var(--bark)"
      style={{ transformOrigin: '770px 1000px', animationDelay: '0.05s', animationDuration: '0.9s' }}
    />
  </g>
);
...
const Wildflowers = ({ palette, density }) => {
  ...
  return (
    <g>
      {flowers.map((f, i) => (
        <Flower key={`${palette}-${density}-${i}`} flower={f} idx={i} density={density} />
      ))}
    </g>
  );
};
...
// ---------- Main scene ----------
const TreeScene = ({ palette = 'spring', density = 'normal', replayKey = 0 }) => {
  return (
    <div className="scene" key={replayKey}>
      <svg viewBox="0 0 1400 1000" preserveAspectRatio="xMidYMax meet"
        ...
        {/* layer order: trunk, branches, canopy on top, then flowers in front of trunk base, grass on top */}
        <Trunk />
        <Branches />
...
Object.assign(window, { TreeScene });
